...
async function detectTFMOBILE(imgToPredict) {
    ...
    const smallImg = tf.image.resizeBilinear(tfImg, [320, 320]) // 600, 450 [320, 320]
    const resized = tf.cast(smallImg, 'float32');
    const tf4d = tf.tensor4d(Array.from(resized.dataSync()), [1, 320, 320, 3]); //730,610 600, 450
...
function renderPredictionBoxes (predictionBoxes, predictionClasses, predictionScores)
{
    ...
    for (let i = 0; i < 99; i++) {
        ...
        const minY = (predictionBoxes[i * 4] * 320).toFixed(0); //730, 610
        const minX = (predictionBoxes[i * 4 + 1] * 320).toFixed(0);
        const maxY = (predictionBoxes[i * 4 + 2] * 320).toFixed(0);
        const maxX = (predictionBoxes[i * 4 + 3] * 320).toFixed(0);
